...
contract DigDragonMineKUB is ERC721Holder, ReentrancyGuard, Ownable {
    ...
    function rewardPerHashPower() public view returns (uint) {
        ...
        //totalRewardPerHashUntilNow = summation of the calculated reward per hashpower for every stake / unstake called
        ...
        (bool result, uint period) = _getCurrentBlock().trySub(
            lastUpdatedRewardBlock
        );

        return
            currentRewardPerHashPower +
            (rewardPerBlock * period * 1e18) /
            totalHashPower;
    }
    ...
    function _earnReward(address _miner) internal {
        ...
        uint supply = address(this).balance;
        ...
        if (rewards > 0 && supply > 0) {
            (uint256 feeRate, uint256 payout) = _calculateFee(rewards);
            if (supply < rewards) {
                (bool success, ) = _miner.call{value: supply}("");
                require(success, "cannot transfer reward");
            } else {
                (bool collected, ) = feeCollector.call{value: feeRate}("");
                require(collected, "cannot correct fee");

                (bool paid, ) = _miner.call{value: payout}("");
                require(paid, "cannot correct fee");
            }
    ...
    function emergencywithdrawKUB() public onlyOwner {
