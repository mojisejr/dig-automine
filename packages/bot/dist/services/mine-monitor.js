...
exports.MineMonitorService = void 0;
const logger_1 = require("../utils/logger");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
class MineMonitorService {
    ...
    async checkMineStatus(mineAddress) {
        try {
            const mineAbi = [
            ...
            this.updateStatusHistory(mineAddress, isActive);
            logger_1.logger.debug(`Mine ${name} status: ${isActive ? 'Active' : 'Inactive'}`, 'MINE_MONITOR');
            return status;
        }
        catch (error) {
            logger_1.logger.error(`Failed to check mine status for ${mineAddress}`, error, 'MINE_MONITOR');
            throw error;
        }
    }
    async checkAllMines() {
        try {
            ...
            logger_1.logger.info(`Mine status - Current: ${currentStatus.name} (${currentStatus.isActive ? 'Active' : 'Inactive'}), Target: ${targetStatus.name} (${targetStatus.isActive ? 'Active' : 'Inactive'})`, 'MINE_MONITOR');
            return { current: currentStatus, target: targetStatus };
        }
        catch (error) {
            logger_1.logger.error('Failed to check all mine statuses', error, 'MINE_MONITOR');
            throw error;
        }
    }
    async shouldSwitchMine() {
        try {
            ...
            logger_1.logger.error('Failed to determine if mine switch is needed', error, 'MINE_MONITOR');
            return {
                shouldSwitch: false,
    ...
    async getMineSwitchRecommendation() {
        try {
            ...
            logger_1.logger.error('Failed to get mine switch recommendation', error, 'MINE_MONITOR');
            throw error;
        }
    }
    loadStatusHistory() {
        try {
            const historyFile = path_1.default.join(__dirname, '../../../../docs/mine-history.json');
            if (fs_1.default.existsSync(historyFile)) {
                const data = JSON.parse(fs_1.default.readFileSync(historyFile, 'utf8'));
                this.statusHistory = new Map(Object.entries(data).map(([addr, history]) => [addr, history]));
                logger_1.logger.debug('Loaded mine status history', 'MINE_MONITOR');
            }
        }
        catch (error) {
            logger_1.logger.error('Failed to load mine status history', error, 'MINE_MONITOR');
        }
    }
    saveStatusHistory() {
        try {
            ...
            fs_1.default.writeFileSync(historyFile, JSON.stringify(data, null, 2));
        }
        catch (error) {
            logger_1.logger.error('Failed to save mine status history', error, 'MINE_MONITOR');
        }
    }
    updateStatusHistory(mineAddress, isActive) {
        ...
        if (!lastEntry || lastEntry.isActive !== isActive) {
            ...
            logger_1.logger.debug(`Status change recorded for mine ${mineAddress}: ${isActive}`, 'MINE_MONITOR');
        }
    }
    analyzeMineTiming(mineAddress) {
        const history = this.statusHistory.get(mineAddress);
    ...
    async getOptimalSwitchTiming() {
        try {
            ...
            logger_1.logger.error('Failed to analyze optimal switch timing', error, 'MINE_MONITOR');
            throw error;
        }
    }
}
exports.MineMonitorService = MineMonitorService;
//# sourceMappingURL=mine-monitor.js.map